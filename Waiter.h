...
// Waiter class
//
// Copyright Tarim - 2011, 2014, 2015
//
...
template <class ALARM_T = int32_t, uint8_t SHIFT_ = 0>
class WAITER {
public:
    //
    ...
    inline void set( const alarm_t duration ) {
        alarmTime = adjust( millis() ) + duration;
    };
    ...
    inline bool alarm( const alarm_t duration = 0 ) {
        return compare( millis(), alarmTime + duration );
    };
    ...
    bool repeat( const alarm_t duration ) {
        if( compare( millis(), alarmTime + duration ) ) {
            alarmTime += duration;
            return true;
        }
        return false;
